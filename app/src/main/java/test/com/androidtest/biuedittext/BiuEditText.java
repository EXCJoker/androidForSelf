package test.com.androidtest.biuedittext;
    ...
    private void setlistener() {
        addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                Log.d("ASD", "CharSequence s = " + s + ", start = " + start + ",before = " + before + ",count = " + count);

                if (cacheStr.length() < s.length()) {
                    int cacheLength = cacheStr.length();
                    int sLength = s.length();
                    char last = 0;
                    if (cacheLength + 1 == sLength) {
                        last = s.charAt(s.length() - 1);
                        update(last, false);
                    } else if (cacheLength + 1 < sLength) {
                        for (int i = cacheLength; cacheLength < sLength; cacheLength++) {
                            last = s.charAt(cacheLength);
                            update(last, false);
                        }
                    } else {
                        last = s.charAt(s.length() - 1);
